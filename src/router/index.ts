...
import No500 from '../components/500.vue'
...
const routes: any = [
  {
    path: '/', component: IndexVue, name: 'Index', redirect: (to: any, next: any) => {
      ...
      if (store.userInfo?.token) {
        if (store.userInfo.accessCode.some(item => item == `${VITE_APP_NAME}.*`)) return '/dashboard/index'
        const fistMeun = routerList.find(x => {
          if (x.children) {
            return x.children.find(y => {
              return store.userInfo?.accessCode.find(z => {
                return y.meta.code == z
              })
            })
          } else {
            return store.userInfo?.accessCode.find(y => y == x.meta.code)
          }
        })
        if (fistMeun) {
  ...
  { path: '/500', component: No500, name: '500', meta: { name: '500' } },
]
...
router.beforeEach((to, from, next) => {
  ...
  if (to.matched.length > 0) {
    ...
    const codeList = store.userInfo?.accessCode as []
    if (token) {
      // 根据路由中meta信息判断是否有权限访问，白名单，超级管理员则放行,
      if (codeList.some(item => item == to.meta.code) || wList.some(item => item == to.path) || store.userInfo?.accessCode.some(item => item == `${VITE_APP_NAME}.*`)) {
        next()
      } else {
        router.push(`/401`)
      }
